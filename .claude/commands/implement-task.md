...
argument-hint: <task-id> (e.g. P0-GPDF-2) [path-or-URL to the task list, if not the default]
---

You are driving a single roadmap task to completion in `living-doc-generator-pdf`.
...
- Do not change externally-visible contracts (`action.yml` inputs/`INPUT_*`, the Action
  output keys `pdf-path`/`html-path`/`report-path`, exit codes `1`-`5`, contract-sensitive
  strings) unless the task explicitly calls for it.
- If a `spec.md` section is being implemented, apply `.claude/rules/docs-lifecycle.md` in
  this same change: move that section's content out of `spec.md` into the live docs.
...
- "invalid input maps to exit code 1" → confirm the `except` branch in `main.run()` maps to
  that exact code, before checking any test.
