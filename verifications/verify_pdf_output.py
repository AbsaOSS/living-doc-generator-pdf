...
def main() -> None:
    ...
    try:
        with open(pdf_path, "rb") as f:
            header = f.read(4)
            if header == b"%PDF":
                print("  ✓ PASS: PDF file has valid magic bytes (%PDF)")
            else:
                print("  ✗ FAIL: PDF file does not start with %PDF magic bytes")
                all_passed = False
    ...
    if report_path.exists():
        print("  ✓ PASS: pdf_report.json exists")
    else:
        print("  ✗ FAIL: pdf_report.json not found")
        all_passed = False
    ...
    if report_path.exists():
        ...
        try:
            ...
            print("  ✓ PASS: pdf_report.json is valid JSON")
